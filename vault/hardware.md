...
### 프리플로우 archon M1 Pro 2 Max 8k

디맥을 할 때 손 회전 각도 때문에 풀배열 키보드가 덜 어색함. 집에 있는 풀배열 키보드는 갈축 55g 키압 키보드인데 전에 쓰던 65% 적축 40g 키보드에 비해 트리거 포인트도 있어서 더 힘이 들어가고 어색함. 이 키보드에 적응하기 보다 낮은 리니어 키 키보드를 찾기로 하고 5시간 정도 찾아봄.
마지막 한 시간 정도에 자석축 키보드를 발견함. 래피트 트리거 기능이 좋아보인다. 32g, 35g 키압이 있어 실제 전자저울에 손가락 힘 빼고 테스트 해보니 32g이 넘기도 해서 고민하다가 32g 키보드에 35g 키 스위치 10개를 주문함. 검은 키보드는 먼지가 눈에 잘 띄어서 밝은 색 계열로 고름.
키보드는 리퍼로 정가 109,000원보다 2만원 정도 싸게 삼. 실제로 받아보니 패키지 말고는 새 제품처럼 보임.
32g 키 스위치가 트래블이 28m 였던가? 짧은 게 느껴짐. 짧은 시간 플레이 해본 경험으로는 긍정적임.

한성컴퓨터에 비슷한 스펙의 키보드가 있었는데, 키 스위치를 따로 팔지 않아서 이쪽으로 정했음.
