...
- 원인: MySQL에서 `foreign_key_checks=OFF` 상태일 때 `DROP FOREIGN KEY` 시 자동 생성 인덱스도 함께 삭제되어, 이후 `RENAME INDEX`가 존재하지 않는 인덱스를 참조
...
나중에 ~/.claud.json에 설정 추가해서 해결 ([[~/.claud.json - mcp 설정|journal.what-i-struggled-brag-til-in#claudjson---mcp-설정]] 참고. 공식문서에 없고 repo 뒤져서 찾음)
...
![nuxt page blink during nav](assets/movs/what-i-struggled-brag-til-in/nuxt-page-blink-during-nav.mp4)
...
error 메시지가 떠서 [[Week 25, 2023 - @babel/preset-env core-js Polyfill|journal.what-i-struggled-brag-til-in#week-25-2023---babelpreset-env-core-js-polyfill]]와 같이 babel build script 수정하여 core-js polyfill 적용하니
...
- Cause: DataTable의 header영역에서 단순히 count만 증가시켜도 전체 Table re-rendering 발생
  ![before performance](./assets/images/what-i-struggled-brag-til-in/20240104-table-rendering-before.png)
- Solution: header 분리
  ![after performance](./assets/images/what-i-struggled-brag-til-in/20240104-table-rendering-after.webp)
...
첫 번째 배포 후 테스트에는 정상 동작, 두 번째 배포 후 테스트에도 정상이었음. 첫 번째 내용은 텍스트와 단순 UI style 수정이었고, [두 번째가 cache bust 관련 bug fix](journal.what-i-struggled-brag-til-in.md#week-51-2023---webpack-tailwindcss-조합에서-appcss-cache-bust가-안됨)였는데...
...
![tailwind - xl:px-8 overwritten with px-6](assets/images/what-i-struggled-brag-til-in/media-query-overwritten-with-non-media-query.webp)
...
|      |                                                                                                                                            |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| from | ![voxmedia__github-action-slack-notify-build](/assets/images/what-i-struggled-brag-til-in/voxmedia__github-action-slack-notify-build.webp) |
| to   | ![slackapi__slack-github-action](/assets/images/what-i-struggled-brag-til-in/slackapi__slack-github-action.webp)                           |
...
[[SPA framework - S3 - CloudFront 배포 시 404 error 처리 | What I struggled 🧗‍♂️/📣 brag In|journal.what-i-struggled-brag-til-in#week-51-2022---spa-framework---s3---cloudfront-배포-시-404-error-처리]]로 세팅된 배포 환경에서 첫 진입 url에 query를 넣을 경우 제대로 전달이 안된다.
...
![](assets/images/what-i-struggled-brag-til-in/scrollbar-width.webp)
...
![index.html cache가 남아 있으면 front 배포 때마다 새로 build 된 assets의 uri을 못 가져온다.](assets/images/what-i-struggled-brag-til-in/cloudfront__invalidate-html-cache-0.webp)
...
![](assets/images/what-i-struggled-brag-til-in/week4-2023__test-server.webp)
...
cypress에 비해 다음 이점이 확연하다. (그리고 [[단점|journal.what-i-struggled-brag-til-in#week-5-2023---playwright-test-code-structuring-draft]]이 드러나는데...)
...
![](assets/images/what-i-struggled-brag-til-in/s3-cloudfront-404-error.webp)

cloudfront에서 403 error를 SPA framework index.html로 처리하도록 해주면 framework에서 404 처리해 줌
![](assets/images/what-i-struggled-brag-til-in/s3-cloudfront-404-custom-setting.webp)
...
![](assets/images/what-i-struggled-brag-til-in/nuxt3-generate__github-actions.webp)  
generate시에
...
| <video width="320" height="240" controls><source src="/assets/movs/what-i-struggled-brag-til-in/tanstack-query__before.webm" type="video/webm"></video> | <video width="320" height="240" controls><source src="/assets/movs/what-i-struggled-brag-til-in/tanstack-query__after.webm" type="video/webm"></video> |
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
