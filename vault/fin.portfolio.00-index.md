...
> 단일 통합본 `fin.portfolio.md`를 대체한다. 이 문서는 입구다. 내용은 아래 9개 문서에 있다.
>
...
- 이 배분에 대한 20년 백테스트 검증: [[B|fin.portfolio.09-backtest]]
...
| [[09 백테스트\|fin.portfolio.09-backtest]]                     | **B**  | 기준안·공격적안 20년 시뮬레이션 결과, 방법론과 한계                                          | 배분·밴드 규칙을 바꾼 뒤 검증하고 싶을 때 |
...
| 백테스트 결과 수치 (XIRR·MDD)                 | B-3, B-4, B-7 (A-4·O-3은 인용)      |
...
- 02를 고치면 → 03 → 05 → 07
- 03을 고치면 → 05·07
- 02·03을 고치면 → 09에서 재검증 여부 확인 (자동 반영되지 않음, 수동 재실행 필요)
- 06·08은 다른 문서의 판단에 영향을 주지 않는다
- 09도 마찬가지로 다른 문서의 판단에 영향을 주지 않는다 — 검증 기록이며 결론의 근거가 아니다.
...
- 2026-09-18 기준안·공격적안 비교 해석 보강(위험 대비 수익, 치우침 요인), A-4에 백테스트 가격표·대출 역캐리 각주 보강 → [[B-5.1|fin.portfolio.09-backtest#b-51-비교가-한쪽으로-치우칠-수-있는-요인]], [[A-4|fin.portfolio.02-allocation#a-4-성향별-조정]]
- 2026-09-18 기준안·공격적안 20년 백테스트 추가 → [[B-1|fin.portfolio.09-backtest#b-1-실행-정보]]
